module Make (CON : Conduit_mirage.S) = struct
  ...
  let split_path path =
    let rec aux = function [] | [ "" ] -> [] | hd :: tl -> hd :: aux tl in
    List.filter
      (fun e -> e <> "")
  ...
  let callback (_, conn_id) req _body =
    ...
    Log.info (fun f ->
        f "Received connection %s on path %s"
          (Cohttp.Connection.to_string conn_id)
          path);
    let segments = split_path path in
    let resp status body_str =
      H.respond ~headers ~status ~body:(Cohttp_lwt.Body.of_string body_str) ()
    in
    match segments with
