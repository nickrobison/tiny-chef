...
let interval =
  let doc = Arg.info ~doc:"Client fetch interval (in seconds)" [ "interval" ] in
  Arg.(value & opt int 5 doc)

let uri_conv = Arg.conv ((fun s -> Ok (Uri.of_string s)), Uri.pp)

let default_target =
  Uri.of_string "https://arepublixchickentendersubsonsale.com"

let target =
  let doc = Arg.info ~doc:"Upstream target to fetch" [ "upstream" ] in
  Arg.(value & opt uri_conv default_target doc)

let http_port =
  let doc = Arg.info ~doc:"HTTP port to listen on" [ "http_port" ] in
