...
let runtime_args =
  [
    runtime_arg ~pos:__POS__ "Unikernel.interval";
    runtime_arg ~pos:__POS__ "Unikernel.target";
    runtime_arg ~pos:__POS__ "Unikernel.http_port";
  ]

let client =
  main ~runtime_args
    ~packages:[ package "duration"; package "cohttp-mirage" ]
    "Unikernel.TinyChef"
    (time @-> conduit @-> mclock @-> http_client @-> job)
...
let () =
  ...
  let job =
    [
      client $ default_time $ conduit $ default_monotonic_clock
