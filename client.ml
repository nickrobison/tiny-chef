...
struct
  ...
  let request ctx uri =
    ...
    | false ->
        Log.err (fun f ->
            f "Received response: %d from %a after %d ms" response_code
              Uri.pp_hum uri (Duration.to_ms duration));
        body
    | true ->
        Log.info (fun f ->
            f "Received response: %d from %a after %d ms" response_code
  ...
  let start ctx uri interval =
    Log.info (fun f ->
        f "Starting requests to %a every %a seconds" Uri.pp uri Duration.pp
          interval);
